...
*   **✅ Verified Gurbani Citations**: Every model tested misquotes Gurbani now and then — an altered word, two lines blended into one, the wrong Ang, the wrong Guru. So after each reply, every line it quotes is checked against GurbaniNow, and a card under the reply shows what the source actually says: the letter-perfect line, its translation, Ang, writer, and raag, with a link to open that Ang. A quote that doesn't match is flagged ("Wording differs from the source", "Found on a different Ang", "Couldn't verify this line") beside the closest real line. The reply itself is never edited, and nothing in a card comes from the model.
...
*   **Checking quotes without trusting the model** (`lib/gurbani/`): the verifier pulls quoted Gurmukhi out of a reply (in a Punjabi reply, only lines closed by `॥`; never greetings or raag headings) and pairs each with the Ang the reply cites. It reads that Ang first — a match there costs no search — then falls back to GurbaniNow's first-letter search, mapping vowel-initial words to the carrier letters (ੲ ੳ ਅ) the index files them under. A quote verifies only when every word appears, in order, in one real line; spelling slips (ਪਸਾਊ for ਪਸਾਉ) still match, but a word swapped or truncated does not. When the source can't be reached, the reply simply gets no card — silence is never reported as "unverified". The tests replay real GurbaniNow answers recorded by `npm run fixtures:gurbani` over 18 real model replies, so scripture in the test data is never typed by hand. The Gurmukhi text utilities are shared with the Shabad identification work.
